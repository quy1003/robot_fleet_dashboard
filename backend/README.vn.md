...
- **Lỗi Port đụng độ khi chạy Cluster**: 
  - *Tại sao?* Ở các ứng dụng Node.js thông thường (như Express), module `cluster` hoạt động bằng cách "bắt cóc" (hijack) module `net`/`http` mặc định: Tiến trình Master sẽ đứng ra đại diện giành cổng 8080 và ném kết nối cho các Worker. Tuy nhiên, **uWebSockets.js** là một thư viện C++ nguyên thủy (native) giao tiếp trực tiếp với hệ điều hành và không sử dụng module `net` của Node.js. Vì vậy, `cluster` hoàn toàn "mù" và không thể đánh chặn uWebSockets.js. Hậu quả là mỗi Worker tự thân đâm xuống hệ điều hành để đòi mở cổng 8080, dẫn đến lỗi đụng độ `EADDRINUSE`.
  - *Giải pháp:* Thay vì dùng tiến trình Master của Node làm đại diện, ta dùng cờ **SO_REUSEPORT** (tính năng của nhân Linux) để nhờ chính hệ điều hành Linux làm người chia tải. Bằng cách truyền tham số `0` vào hàm `listen()`, uWebSockets.js sẽ bảo Linux cho phép nhiều Worker cùng bám vào một cổng: `app.listen(PORT, 0, (token) => {...})`.
